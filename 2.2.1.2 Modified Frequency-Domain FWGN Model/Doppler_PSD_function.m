...
%   'laplacian' : S(f)=1./sqrt(1-f0.^2).*(exp(-sqrt(2)/sigma*abs(acos(f0)-phi))+exp(-sqrt(2)/sigma*abs(acos(f0)-phi)))
%                 with sigma(angle spread of UE) and phi(=DoM-AoA) in the case of 'laplacian' Doppler type
...
%   f0          : Normalized Doppler frequency defined as f0=f/fm 
